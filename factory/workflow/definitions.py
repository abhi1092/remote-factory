...
def evolve_workflow() -> Workflow:
    ...
    nodes["baseline"] = FnNode(
        ...
        command=(
            ...
            "CEO must call get_benchmark_info() via MCP, extract domain context to benchmark_info.json, "
        ...
        notes=(
            ...
            "2. Extract structured domain context from the get_benchmark_info() response and write to "
            ".factory/baseline/benchmark_info.json as JSON. Include all fields returned by the MCP tool "
            "(benchmark_name, description, task_type, input_format, output_format, constraints, "
            "optimization_target, evaluation_criteria, example_inputs — whatever the tool provides).\n"
            ...
            "8. Copy the eval result to .factory/experiments/000/eval_before.json "
        ...
        writes={
            ...
            ".factory/baseline/benchmark_info.json",
    ...
    nodes["researcher"] = AgentNode(
        ...
        prompt_template=(
            ...
            "Read .factory/baseline/benchmark_info.json for structured domain context: "
            "problem description, evaluation criteria, constraints, and optimization target. "
            "Use this context to understand the problem class, optimization objectives, and solution space. "
            "Search the web for optimization techniques, heuristics, and algorithmic strategies "
            ...
            "candidate optimization techniques ordered by expected impact (grounded in evaluation criteria), "
    ...
    nodes["strategist"] = AgentNode(
        ...
        prompt_template=(
            ...
            "Read domain context at .factory/baseline/benchmark_info.json — "
            "use evaluation_criteria to align hypotheses with what the benchmark rewards, "
            "and constraints to ensure modifications stay within limits. "
            ...
            "If the last 2 experiments were both reverted AND from the same FEEC family "
            "(same Category field: algorithm-change, parameter-tuning, data-structure, or initialization), "
            "note this — the CEO will trigger fresh research. "
    ...
    nodes["gate_strategy"] = GateNode(
        ...
        gate_prompt=(
            ...
            "5) Check stuck detection: Read .factory/results.tsv and extract the Category field "
            "from the last 2 experiment hypotheses. If BOTH of the last 2 experiments were REVERT "
            "AND had the same Category (same FEEC family), trigger MANDATORY RELOOP to researcher "
            "for fresh perspective instead of proceeding to builder.\n"
            "PROCEED if hypothesis is sound and not stuck. "
            "RELOOP to strategist if hypothesis is vague or wrong category. "
            "RELOOP to researcher if stuck (2 consecutive same-family reverts)."
        ),
        reads={".factory/strategy/current.md"},
    )
    ...
    nodes["health_checker"] = AgentNode(
        ...
        prompt_template=(
            ...
            "The JSON MUST include at minimum: combined_score (float), validity (bool), eval_time (float). "
            "Include ALL additional fields returned by evaluate_solution() verbatim — "
            "do NOT filter, rename, or hardcode benchmark-specific metrics. "
            "The schema is determined by the MCP evaluator's response, not by this template.\n"
